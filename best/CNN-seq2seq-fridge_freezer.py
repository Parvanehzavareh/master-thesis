...
window_size = 91      # number of time steps per input sample (4200 sec)
step_size = 1         # sliding window step
columns = ["fridge_freezer",]
...
#appliance = 'fridge_freezer'
    ...
    y = np.vstack(y_all)  # Stack target arrays along axis 0
    ...
    model.add(Flatten())
    model.add(Dense(dense_layer_size, activation='relu')) #1024, 512, 256, 128
    if droup_out!=0:
        model.add(Dropout(droup_out))    
    model.add(Dense(window_size, activation='linear'))  
    model.add(Reshape((window_size,))) 
    ...
    if normal == "min_max_ratio":
        ...
        #y_test_reconstructed2 = test_segment_df[appliance].values
        ...
        mean_test = df_test_denormalized["aggregate"].mean()
        std_test = df_test_denormalized["aggregate"].std()
        mean_test_y = df_test_denormalized[appliance].mean()
        std_test_y = df_test_denormalized[appliance].std()
        start_time =start_time_dict[appliance]
        duration = duration_dict[appliance]
        end_time = start_time + pd.Timedelta(hours=duration)
        ...
        #denormalization
        X_test_denormalized = X_test_reconstructed*std_test+mean_test
        y_test_denormalized = y_test_reconstructed*std_test_y+mean_test_y
        y_pred_denormalized = y_pred_reconstructed*std_appliance_total+mean_appliance_total
        ...
        min_test = df_test_denormalized["aggregate"].min()
        max_test = df_test_denormalized["aggregate"].max()
        min_test_y = df_test_denormalized[appliance].min()
        max_test_y = df_test_denormalized[appliance].max()
        start_time =start_time_dict[appliance]
        duration = duration_dict[appliance]
        end_time = start_time + pd.Timedelta(hours=duration)
        ...
        #denormalization
        X_test_denormalized = X_test_reconstructed*(max_test-min_test)+min_test
        y_test_denormalized = y_test_reconstructed*(max_test_y-min_test_y)+min_test_y
        y_pred_denormalized = y_pred_reconstructed*(max_appliance_total-min_appliance_total)+min_appliance_total
