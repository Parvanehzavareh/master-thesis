...
folders = [
    ...
    "CNN - dt=60 - dense=256 - batchsize=1024 - dropout=0.25",
    "CNN - dt=60 - dense=256 - batchsize=1024 - dropout=0.5"
]
...
folder1 = {
        "CNN - dt=60 - dense=256 - batchsize=1024" : "CNN - dt=60 - dense=256 - batchsize=1024 - dropout=0",
        "CNN - dt=60 - dense=256 - batchsize=1024 - dropout=0.25" : "CNN - dt=60 - dense=256 - batchsize=1024 - dropout=0.25",
        "CNN - dt=60 - dense=256 - batchsize=1024 - dropout=0.5" : "CNN - dt=60 - dense=256 - batchsize=1024 - dropout=0.5"
}
dropouts = [folder1[folder].split("dropout=")[1] for folder in folders]

# Initialize a dictionary to store data
data_dict = {appliance: {method: {dropout: None for dropout in dropouts} for method in methods} for appliance in appliances}

# Parse CSV files from each folder
parent_dir = os.path.abspath(os.path.join(os.getcwd(), ".."))  # Go one folder back
for folder in folders:
    dropout = folder1[folder].split("dropout=")[1]  # Extract batch size
    for file in os.listdir(os.path.join(parent_dir, folder)):
        if file.endswith(".csv") and file!="comparison_metrics.csv":
            ...
            if appliance in appliances and method in methods:
                data_dict[appliance][method][dropout] = df
...
for i, appliance in enumerate(appliances):
    for j, method in enumerate(methods):
        ...
        for dropout in dropouts:
            df = data_dict[appliance][method][dropout]
...
# Add a main title
fig.suptitle("Comparison of SAE & MAE Across Different dropout ratios", fontsize=16)
...
results_df = results_df.sort_values(by=["appliance", "method", "dropout"])
