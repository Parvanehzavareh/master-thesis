...
folders = [
    "CNN - dt=60 - dense=256 - batchsize=1024 - window_duration=23x60",
    "CNN - dt=60 - dense=256 - batchsize=1024 - window_duration=45x60",
...
folder1 = {
    "CNN - dt=60 - dense=256 - batchsize=1024 - window_duration=23x60" : "CNN - dt=60 - dense=256 - batchsize=1024 - window_duration=23x60",
    "CNN - dt=60 - dense=256 - batchsize=1024 - window_duration=45x60" : "CNN - dt=60 - dense=256 - batchsize=1024 - window_duration=45x60",
    "CNN - dt=60 - dense=256 - batchsize=1024" : "CNN - dt=60 - dense=256 - batchsize=1024 - window_duration=69x60",
}
window_durations = [folder1[folder].split("window_duration=")[1] for folder in folders]

# Initialize a dictionary to store data
data_dict = {appliance: {method: {window_duration: None for window_duration in window_durations} for method in methods} for appliance in appliances}

# Parse CSV files from each folder
parent_dir = os.path.abspath(os.path.join(os.getcwd(), ".."))  # Go one folder back
for folder in folders:
    window_duration = folder1[folder].split("window_duration=")[1]  # Extract batch size
    for file in os.listdir(os.path.join(parent_dir, folder)):
        if file.endswith(".csv") and file!="comparison_metrics.csv":
            ...
            if appliance in appliances and method in methods:
                data_dict[appliance][method][window_duration] = df
...
for i, appliance in enumerate(appliances):
    for j, method in enumerate(methods):
        ...
        for window_duration in window_durations:
            df = data_dict[appliance][method][window_duration]
        ...
        batch_indices = window_durations
...
# Add a main title
fig.suptitle("Comparison of SAE & MAE Across Different window durations", fontsize=16)
...
results_df = results_df.sort_values(by=["appliance", "method", "window duration"])
