...
folders = [
    "CNN - dt=60 - dense=256 - batchsize=1024 - lr=0.0001",
    "CNN - dt=60 - dense=256 - batchsize=1024 - lr=0.0005",
    "CNN - dt=60 - dense=256 - batchsize=1024",
    "CNN - dt=60 - dense=256 - batchsize=1024 - lr=0.005",
]
...
folder1 = {
    "CNN - dt=60 - dense=256 - batchsize=1024 - lr=0.0001" : "CNN - dt=60 - dense=256 - batchsize=1024 - lr=0.0001",
    "CNN - dt=60 - dense=256 - batchsize=1024 - lr=0.0005" : "CNN - dt=60 - dense=256 - batchsize=1024 - lr=0.0005",
    "CNN - dt=60 - dense=256 - batchsize=1024" :             "CNN - dt=60 - dense=256 - batchsize=1024 - lr=0.001",
    "CNN - dt=60 - dense=256 - batchsize=1024 - lr=0.005" :  "CNN - dt=60 - dense=256 - batchsize=1024 - lr=0.005",
}
lrs = [folder1[folder].split("lr=")[1] for folder in folders]

# Initialize a dictionary to store data
data_dict = {appliance: {method: {lr: None for lr in lrs} for method in methods} for appliance in appliances}

# Parse CSV files from each folder
parent_dir = os.path.abspath(os.path.join(os.getcwd(), ".."))  # Go one folder back
for folder in folders:
    lr = folder1[folder].split("lr=")[1]  # Extract batch size
    for file in os.listdir(os.path.join(parent_dir, folder)):
        if file.endswith(".csv") and file!="comparison_metrics.csv":
            ...
            if appliance in appliances and method in methods:
                data_dict[appliance][method][lr] = df
...
for i, appliance in enumerate(appliances):
    for j, method in enumerate(methods):
        ...
        mae_values = []
        for lr in lrs:
            df = data_dict[appliance][method][lr]
...
# Add a main title
fig.suptitle("Comparison of SAE & MAE Across Different learning rate", fontsize=16)
...
results_df = results_df.sort_values(by=["appliance", "method", "learning rate"])
