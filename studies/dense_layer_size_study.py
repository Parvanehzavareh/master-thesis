...
folders = [
    ...
    "CNN - dt=60 - dense=256,128 - batchsize=1024",
    "CNN - dt=60 - dense=512 - batchsize=1024"
]
...
layer_sizes = [folder.split("dense=")[1].split(" ")[0] for folder in folders]

# Initialize a dictionary to store data
data_dict = {appliance: {method: {layer_size: None for layer_size in layer_sizes} for method in methods} for appliance in appliances}

# Parse CSV files from each folder
parent_dir = os.path.abspath(os.path.join(os.getcwd(), ".."))  # Go one folder back
for folder in folders:
    layer_size = folder.split("dense=")[1].split(" ")[0]  # Extract batch size
    ...
    for file in os.listdir(os.path.join(parent_dir, folder)):
        if file.endswith(".csv") and file!="comparison_metrics.csv":
            ...
            if appliance in appliances and method in methods:
                data_dict[appliance][method][layer_size] = df
...
for i, appliance in enumerate(appliances):
    for j, method in enumerate(methods):
        ...
        for layer_size in layer_sizes:
            df = data_dict[appliance][method][layer_size]
            print(df.columns)
...
# Add a main title
fig.suptitle("Comparison of SAE & MAE Across Different Dense Layer Sizes", fontsize=16)
...
results_df = results_df.sort_values(by=["appliance", "method", "dense layer size"])
