...
folders = [
    ...
    "CNN - dt=60 - dense=256 - batchsize=1024 - loss_fun=MAE",
]
...
folder1 = {
    "CNN - dt=60 - dense=256 - batchsize=1024" : "CNN - dt=60 - dense=256 - batchsize=1024 - loss_fun=MSE",
    "CNN - dt=60 - dense=256 - batchsize=1024 - loss_fun=MAE" : "CNN - dt=60 - dense=256 - batchsize=1024 - loss_fun=MAE",
}
loss_funs = [folder1[folder].split("loss_fun=")[1] for folder in folders]

# Initialize a dictionary to store data
data_dict = {appliance: {method: {loss_fun: None for loss_fun in loss_funs} for method in methods} for appliance in appliances}

# Parse CSV files from each folder
parent_dir = os.path.abspath(os.path.join(os.getcwd(), ".."))  # Go one folder back
for folder in folders:
    loss_fun = folder1[folder].split("loss_fun=")[1]  # Extract batch size
    for file in os.listdir(os.path.join(parent_dir, folder)):
        if file.endswith(".csv") and file!="comparison_metrics.csv" and file!='min_max_summary.csv': 
            ...
            if appliance in appliances and method in methods:
                data_dict[appliance][method][loss_fun] = df
...
for i, appliance in enumerate(appliances):
    for j, method in enumerate(methods):
        ...
        for loss_fun in loss_funs:
            df = data_dict[appliance][method][loss_fun]
...
# Add a main title
fig.suptitle("Comparison of SAE & MAE Across Different Loss Functions", fontsize=16)
...
results_df = results_df.sort_values(by=["appliance", "method", "loss function"])
